...
- For the demo, prefer LLM quota availability, latency, and cost over maximum routing quality, provided deterministic validation prevents incorrect mutations or unverified factual output.
...
- Use Gemini's free tier through the Interactions API as the initial LLM provider behind a narrow provider interface. Default to the stable `gemini-3.5-flash-lite` model because quota availability, latency, and cost matter more than maximum routing quality for this demo; keep the model ID configurable through an environment variable. Deterministic validation and verified application results must continue to contain any diminished model capability. Only one provider implementation is in demo scope. The free tier's data-use terms must be documented. See the [Gemini API reference](https://ai.google.dev/api), [latest-model guidance](https://ai.google.dev/gemini-api/docs/latest-model), and [pricing](https://ai.google.dev/gemini-api/docs/pricing).
...
5. **Conversational tool-calling interface**
   - Status: implemented locally and verified with 163 tests, Ruff lint/format, JavaScript syntax, dependency, wheel/asset packaging, and Render Blueprint checks. The default model is now `gemini-3.5-flash-lite`; live model-quality and quota validation remains required. Human Gemini-key configuration, live-Gemini/Bruno/browser validation, review, commit/push, CI, and deployed verification remain.
